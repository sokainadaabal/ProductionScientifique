...
search() {
  if (this.nom != "") {
    this.prof = this.prof.filter(res => {
      return res.id.toString().match(this.nom)|| res.type_revu.match(this.nom) || res.titre.match(this.nom) || res.annee.match(this.nom)|| res.journal.match(this.nom) ;
    }
    )
  }
  else if (this.nom == "") {
